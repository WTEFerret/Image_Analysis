...
The output is organized into directories for each series.
		...
		try:
			...
			if 'v' in keys:
				if not paras['v'] in positions.keys():
					...
					create_directory(positions[paras['v']])
			else:
				positions[0] = os.path.join(out_dir,get_nd_pname(nd2, 0))
				create_directory(positions[0])

			if 'v' in keys:
				imsave(os.path.join(positions[paras['v']], img_name), frame, check_contrast=False)
			else:
				imsave(os.path.join(positions[0], img_name), frame, check_contrast=False)
